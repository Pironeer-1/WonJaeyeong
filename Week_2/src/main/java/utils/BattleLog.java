...
public class BattleLog {
  ...
  public void printPlayerInfo(Player player) {
    System.out.println("현재 유저 : 체력 " + player.getHp() + ", 공격력 " + player.getAttackPower() + ", 마법력 " + player.getMagicPower());
  }

  public void printEnemyInfo(Enemy enemy) {
    System.out.println("적 : 체력 " + enemy.getHp() + ", 공격력 " + enemy.getAttackPower() + ", 방어력 " + enemy.getDefense() + ", 마법방어력 " + enemy.getMagicDefense());
  }
  ...
  public void printPlayerChoice(int idx) {
    System.out.println((idx + 1) + "번 플레이어의 차례입니다. 행동을 선택하세요. (1: 스테이터스 확인 + 일반 공격, 2: 기본 공격, 3: 마법 공격, 4: 체력 회복, exit: 종료)");
  }
  ...
  public void printRemainingHp(int idx, int damage, int hp) {
    System.out.println((idx + 1) + "번 유저에게 " + damage + "의 데미지. 적의 공격으로 현재 남은 체력은 " + hp + "입니다.");
  }
